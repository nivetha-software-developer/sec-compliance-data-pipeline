...
forms = ['25-NSE', '6-K', '8-K', '8-K12B', '25', '425', '486BPOS', 'DEFA14A',
            'DEFM14A', 'DEFM14C', 'F-1', 'F-4', 'N-2', 'N-23C3A', 'PRE 14A', 'PRE 14C',
                'PREM14A', 'PREM14C', 'RW', 'S-1', 'S-3D', 'S-4', 'S-8 POS', 'S-11', 'SC 13E3',
                    'SC 14D9', 'SC TO-C', 'SC TO-I', 'SC TO-T', 'SC 13E4']    # Totally " 30 " Form Types for CA
...
# Logger Condition
# ----------------------------------
logger_man = Logger(f'SEC_CA_{str(current_run)}', root_folder=root_folder, todayDate=todayDate)
...
excel_filename = f"CA{todayDate}_{current_run}.xlsx"
ca_excel_path = os.path.join(run_folder, excel_filename)
...
def process_incremental_from_scriptlog1():
    ...
    forms_to_remove = ['253G2', 'S-1MEF', 'S-3DPOS', 'F-4/A', 'SC 13E3/A', 'F-10']
    ...
    for folder_num in range(1, 10):
        ...
        try:
            ...
            try:
                ...
                CA_Validator.validator_check_for_yesterday_path(
                    yesterdayDate,
                    folder_num,
                    folder_path,
...
def check_missing_file_validation(run_folder_path):
    ...
    for file in os.listdir(run_folder_path):
        if file.startswith("CA") and file.endswith(".xlsx"):
            ca_excel = os.path.join(run_folder_path, file)
            break

    if not ca_excel:
        logger("CA Excel file not found", Logger.ERROR)
        return False
    ...
    logger(f"Missing_files_list: {missing_files}", Logger.INFO)
    # -----------------------------
    # 5. Result
    # -----------------------------
    if missing_files:
        ...
        for m in missing_files:
            ...
            if not row.empty:
                ...
                CA_Validator.download_missing_pdfs(driver, url, run_folder_path, m, config, temp_pdf_download_dir)
        return True
    else:
...
def can_send_mail():
    now = datetime.now(ZoneInfo("Asia/Kolkata"))  # IST
...
def send_mail():       
    ...
    try:
        postmark.emails.send(
            From='XXX@gmail.com',
            To='YYY@gmail.com',
            Subject='SEC_CA - LC [Server_name] || Success!',
            HtmlBody=f"""
...
try:
    ...
    with open(log_file_path, "a", encoding="utf-8") as log_file:
        for index, record in enumerate(records, start=1):
            ...
            # ----------------------------
            now = datetime.now()
            unique_id_val = ("CA" + now.strftime("%y%m%d%H%M%S") + now.strftime("%f")[:8])
    ...
    if str(current_run) == "1":
        ...
        CA_Validator.validator_check_11(
            todayDate,
            target_folder,
            currentDatetimeInDash,
            forms
    ...
    postmark.emails.send(
        From='XYZ@gmail.com',
        To='XXX@gmail.com',
        Subject='SEC_CA - LC [Server_name] || Alert!',
        HtmlBody=f'There are some issues were encountered. Check Log file. - CA : {current_run}.',
        Attachments=[log_file_name]
    )
finally:
    # ========================
